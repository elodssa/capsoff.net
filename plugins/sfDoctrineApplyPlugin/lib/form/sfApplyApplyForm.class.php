...
class sfApplyApplyForm extends ProfileForm
{
  ...
  public function configure()
  {
    ...
    $this->widgetSchema->setLabels
    		(array('username' => 'Логин для авторизации',
    		       'fullname' => 'Логин, который будут видеть остальные',
    		       'password' => 'Пароль',
    		       'password2' => 'Повторите пароль',
    		       'email2' => 'Повторите Email',
    		       'show_email' => 'Показывать адрес почты остальным?',
    		       'avatar'     => 'Аватарка (не обязательно)',
    		       'captcha' => 'Доказательство, что вы человек'));

	$this->validatorSchema['username']->setMessage('required', 'Введите логин');
	$this->validatorSchema['fullname']->setMessage('required','Введите логин');
	$this->validatorSchema['password']->setMessage('required', 'Введите пароль');
	$this->validatorSchema['password2']->setMessage('required','Введите повтор пароля');
	...
	$this->validatorSchema['email2']->setMessage('required','Введите адрес почты');
	$this->validatorSchema['captcha']->setMessage('invalid', 'Неверный код с картинки');
  }
